    ...
    <div className="grid gap-5" style={{ gridTemplateColumns: '1.3fr 1fr' }}>
      <Card className="p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-[15px] font-bold text-gray-900 tracking-tight">🔔 Action Required</h3>
            <p className="text-[11px] text-gray-400 mt-0.5">AI-generated follow-up priorities</p>
          </div>
            ...
            <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '10px 12px', background: t.bg, borderRadius: 12, borderLeft: `3px solid ${t.border}` }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  ...
                  <span style={{ fontSize: 10, color: t.color, fontWeight: 600, whiteSpace: 'nowrap', marginLeft: 8 }}>{t.label}</span>
                </div>
                <span style={{ fontSize: 11, color: '#6b7280' }}>{t.detail}</span>
              </div>
            </div>
          ))}
        </div>
        <button className="w-full mt-4 py-2.5 rounded-xl text-[12px] font-bold text-white" style={{ background: `linear-gradient(135deg, ${gradientFrom}, ${gradientTo})` }}>
          View All Tasks in CRM →
        </button>
      </Card>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        <Card className="p-5">
          <h3 className="text-[14px] font-bold text-gray-900 mb-3">Pipeline at a Glance</h3>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            {pipeline.map((p) => (
              <div key={p.label} style={{ background: p.bg, borderRadius: 12, padding: '12px 14px', textAlign: 'center' }}>
                <div style={{ fontSize: 28, fontWeight: 900, color: p.color }}>{p.val}</div>
                <div style={{ fontSize: 10, fontWeight: 600, color: '#6b7280', marginTop: 2 }}>{p.label}</div>
              </div>
            ))}
          </div>
        </Card>
        <Card className="p-5">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-[14px] font-bold text-gray-900">📆 Today&apos;s Appointments</h3>
            <span className="text-[11px] text-gray-400">Mon, Mar 30</span>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {appointments.map((a, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 10px', background: a.bg, borderRadius: 10 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: a.color, whiteSpace: 'nowrap' }}>{a.time}</div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: '#111827' }}>{a.name}</div>
