...
export default function ContentCalendar() {
  ...
  function setCaption(k: string, v: string) {
            const m = Object.assign({}, captions);
            m[k] = v;
            setCaptions(m);
  }
            ...
            <div style={{ background: 'linear-gradient(135deg,rgba(14,47,68,0.95) 0%,rgba(21,74,103,0.9) 100%)', border: '1px solid rgba(121,169,209,0.2)', borderRadius: '16px', padding: '24px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
                                      <div>
                                                ...
                                                <p style={{ color: '#6E93A8', fontSize: '13px', margin: '4px 0 0' }}>Plan and schedule your content</p>p>
                                      </div>div>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                          <button onClick={prev} style={btnStyle as React.CSSProperties}>&lsaquo;</button>button>
                                          <span style={{ color: '#F4FBFF', fontSize: '15px', fontWeight: 600, minWidth: '150px', textAlign: 'center' }}>
                                                {MONTH_NAMES[month]} {year}
                                          </span>span>
                                          <button onClick={next} style={btnStyle as React.CSSProperties}>&rsaquo;</button>button>
                                </div>div>
                        </div>div>
                        ...
                        {DAYS.map(function(dn) {
                            ...
                            return (
                                              <div key={dn} style={{ background: 'rgba(7,28,42,0.4)', border: '1px solid rgba(121,169,209,0.12)', borderRadius: '10px', padding: '10px', minHeight: '160px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                                                    {entries.length === 0
                                                                          ? <div style={{ color: '#6E93A8', fontSize: '11px', textAlign: 'center', paddingTop: '16px', opacity: 0.4 }}>No posts</div>div>
                                                                  ...
                                                                  return (
                                                                    <div key={i}>
                                                                                            ...
                                                                                            <div style={{ background: c + '22', border: '1px solid ' + c + '44', borderRadius: '6px', padding: '4px 6px', marginBottom: '4px' }}>
                                                                                                                      <div style={{ fontSize: '10px', fontWeight: 700, color: c }}>{p ? p.platform : ''} - {p ? p.type : ''}</div>div>
                                                                                                                      <div style={{ fontSize: '10px', color: '#B8D3E4', marginTop: '2px', lineHeight: '1.3' }}>{p ? p.topic : ''}</div>div>
                                                                                                  </div>div>
                                                                    </div>div>
                                                                  );
                                                    })
                                                            ...
                                                            <div style={{ marginTop: 'auto', paddingTop: '6px' }}>
                                                                            <div style={{ fontSize: '9px', color: '#6E93A8', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '4px' }}>Caption</div>div>
                                                                            <textarea
                                                                                                    value={captions[ck] || ''}
                                                                                                    onChange={function(e) { setCaption(ck, e.target.value); }}
                        ...
                        {[{l:'Instagram',c:'#e879f9'},{l:'Facebook',c:'#3b82f6'},{l:'TikTok',c:'#06b6d4'},{l:'Email',c:'#f59e0b'}].map(function(item) {
                            return (
                                              <div key={item.l} style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                                            ...
                                                            <span style={{ color: '#6E93A8', fontSize: '11px' }}>{item.l}</span>span>
                                              </div>div>
                                            );
            })}
                  </div>div>
