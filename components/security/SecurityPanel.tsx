...
/**
 * Two-factor (TOTP) enrollment UI.
 *
 * Shared by the staff route (/security, standalone full-screen) and the
 * client portal route (/client/security, embedded so it keeps the client's
 * own nav + portal chrome instead of the staff dashboard shell).
 */
export default function SecurityPanel({ embedded = false }: { embedded?: boolean }) {
  ...
  const inner = (
    <div className="w-full max-w-xl flex flex-col gap-6">
      <header>
        <h1 className="text-3xl font-bold tracking-tight text-white">Account security</h1>
          ...
          <div className="mt-4 flex flex-col gap-3">
            {factors.map((f) => (
              <div key={f.id} className="flex items-center justify-between rounded-xl bg-neutral-800/60 border border-neutral-700 p-3">
                <div>
                  <div className="text-sm font-semibold text-white">{f.friendly_name || 'Authenticator app'}</div>
      ...
      {/* Standalone (staff) view gets a back link; in the client portal the
  ...
  // Embedded: sit inside the portal's own chrome/background (keeps the
  // client's nav). Standalone: full-screen dark page for staff.
  if (embedded) {
