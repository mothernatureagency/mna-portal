...
async function initSchema() {
        ...
        const statements = [
                  ...
                  // Custom clients — Prime IV locations added on the fly by staff
                  // (just a name; the rest is generated by makePrimeIVClient).
                  ...
                  `alter table custom_clients add column if not exists industry text`,
                  `alter table custom_clients add column if not exists brand_from text`,
                  `alter table custom_clients add column if not exists brand_to text`,
                  `alter table custom_clients add column if not exists notes text`,
                  ...
                  EXCEPTION WHEN others THEN NULL;
                  END $$`,
                  // ── AI CRM (Revive / GoHighLevel) ──────────────────────────
                  // One row per connected Revive/GHL subaccount (location).
                  // token_encrypted holds the location's Private Integration
                  // token, AES-256-GCM encrypted server-side (lib/ai-crm/crypto).
                  `create table if not exists ghl_locations (
                        ...
                        response_delay_seconds integer not null default 60,
                        ...
                        gbp_auth_email text,
                        review_auto_enabled boolean not null default false,
                        review_auto_3_star boolean not null default false,
                        created_at timestamptz not null default now(),
                        updated_at timestamptz not null default now()
                  )`,
                  // Per-contact AI state within a location (human takeover,
                  // pauses, opt-outs). Tenant-scoped by ghl_location_id.
                  `create table if not exists ai_conversation_state (
                        ...
                        updated_at timestamptz not null default now(),
                        unique (ghl_location_id, contact_id)
                  )`,
                  // One row per inbound message the AI layer has seen.
                  // unique(ghl_location_id, external_message_id) is the
                  ...
                  `create table if not exists ai_messages (
                        ...
                        created_at timestamptz not null default now(),
                        updated_at timestamptz not null default now(),
                        unique (ghl_location_id, external_message_id)
                  )`,
                  ...
                  `create index if not exists ai_audit_msg_idx on ai_audit_logs (message_id)`,
                  // Google Reviews AI module queue (separate from CRM AI but
                  // shares locations, audit logs and the approval dashboard).
                  `create table if not exists ai_review_queue (
                        ...
                        created_at timestamptz not null default now(),
                        updated_at timestamptz not null default now(),
                        unique (ghl_location_id, google_review_name)
                  )`,
